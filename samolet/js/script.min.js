...
new Vue({
	...
	data: {
		basePrice: 210589,
		selectedCheckboxProducts:[],
		selectedRadioProducts: {
			engines: 0,
			painting: 0,
			cabine: 0,
			basic: 0,
	...
	methods: {
		...
		addPrice(refName){
			var htmlElement = this.$refs[refName];
			var price = parseInt(htmlElement.dataset.price);
				if(htmlElement.type === "radio"){
				console.log(this.selectedRadioProducts[htmlElement.name]);
				this.selectedRadioProducts[htmlElement.name] -= this.selectedRadioProducts[htmlElement.name];
				this.selectedRadioProducts[htmlElement.name] += price;
			} else if(htmlElement.type === "checkbox"){
				if (htmlElement.checked){
					var priceObject = {id: htmlElement.id, price};
					this.selectedCheckboxProducts.push(priceObject);
				} else {
					var newSelectedCheckboxProducts = this.selectedCheckboxProducts.filter(function(el){
						return el.id != htmlElement.id;
						});
					this.selectedCheckboxProducts = newSelectedCheckboxProducts;
				}
			}
		}
	},
	computed: {
		...
		optionalPrice(){
			var summa = 0;
			for(var item of this.selectedCheckboxProducts){
				summa += item.price;
			}
			for(var item in this.selectedRadioProducts){
					summa += this.selectedRadioProducts[item];
			}
			return summa;
		}
	}
